...
$akses_menu = $this->db->query("SELECT * FROM akses_menu WHERE id_user = '$id'")->getRow();

$main_menu = explode(',', $akses_menu->menu);
$main_menu = $this->db->table("app_menu")->where('status', 1)->whereIn('id', $main_menu)->orderBy('posisi', 'ASC')->get()->getResult();
$child = explode(',', $akses_menu->child);
...
<head>
    ...
    <title><?php echo $title; ?> - FLOUR POS -</title>

    <link rel="shortcut icon" href="/assets/img/logo-flour.jpg" type="image/x-icon" />
                    ...
                    <ul class="menu">
                        ...
                        <?php foreach ($main_menu as $key) : ?>
                            <?php if ($key->has_child == 1) {
                                $sub = $this->db->table("app_child_menu")->whereIn('id', $child)->where('id_app_menu', $key->id)->where('status', 1)->orderBy('posisi', 'ASC')->get()->getResult(); ?>
                                <li class="sidebar-item has-sub <?php if ($menu == $key->link) : ?>active<?php endif ?>">
                                    <a href="/<?php echo $key->link; ?>" class="sidebar-link">
                                        <i class="<?php echo $key->icon; ?>"></i>
                                        <span><?php echo $key->nama_menu; ?></span>
                                    </a>
                                    <ul class="submenu">
                                        <?php foreach ($sub as $s) : ?>
                                            <li class="submenu-item <?php if ($submenu == $s->link) : ?>active<?php endif ?>">
                                                <a href="/<?php echo $s->link; ?>" class="submenu-link"><?php echo $s->nama_child; ?></a>
                                            </li>
                                        <?php endforeach; ?>
                                    </ul>
                                </li>
                            <?php } else { ?>
                                <li class="sidebar-item <?php if ($menu == $key->link) : ?>active<?php endif ?>">
                                    <a href="/<?php echo $key->link; ?>" class="sidebar-link">
                                        <i class="<?php echo $key->icon; ?>"></i>
                                        <span><?php echo $key->nama_menu; ?></span>
                                    </a>
                                ...
                                <a href="#" data-bs-toggle="dropdown" aria-expanded="false" class="">
                                    <div class="user-menu d-flex">
                                        <div class="user-name text-end me-3">
                                            <h6 class="mb-0 text-gray-600"><?php echo $this->session->get('nama'); ?></h6>
                ...
                <div class="modal-body">
                    <form action="javascript:void(0)" id="formp" autocomplete="off">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="namatoko" placeholder="Masukkan nama toko" value="<?php echo $this->session->get('nama_toko'); ?>">
                            <div class="invalid-feedback"></div>
                        </div>
                        <div class="mb-3">
                            <label for="nohptoko" class="form-label">Nomor HP</label>
                            <input type="number" class="form-control" name="nohptoko" placeholder="Masukkan nomor hp toko" value="<?php echo $this->session->get('nohp_toko'); ?>">
                            <div class="invalid-feedback"></div>
                        </div>
                        <div class="mb-3">
                            <label for="emailtoko" class="form-label">E-mail</label>
                            <input type="text" class="form-control" name="emailtoko" placeholder="Masukkan e-mail toko" value="<?php echo $this->session->get('email_toko'); ?>">
                            <div class="invalid-feedback"></div>
                        </div>
                </div>
